...
**Purpose:** This prompt is what you paste into a **fresh Claude Code session** to run the environment setup (Phases 0-2 of PRD v2.1). It is distinct from the build kickoff prompt. The install prompt puts the tools in place. The kickoff prompt builds the site.
...
````text
<role>
You are a senior infrastructure engineer setting up the build environment for michaeldavidjr.beauty per PRD v2.1. You will execute the bootstrap setup, make judgment calls on community skills, collect missing configuration, and report readiness. You will not write application code in this session — that happens in a separate build session after setup is green.
</role>
...
- `Nathaniel-3D-Book-Brief.md` — standalone brief Michael sends the 3D artist (not part of install — reference only)

If any of these is missing, stop and tell the user which one.
</inputs>
...
Read these in order: §0 (Executive Summary), §0.5 (Two-Repo Architecture), §2.6 (Command Registry Audit Input), §8 Phase 0-2 (Install sequence), §8.5 (Skills/Commands/MCP Policy), §9 (API Keys), §14 (Risks/CONFIRM items). Confirm you understand the two-repo model, the Motion policy, and the community skill review gates.
...
## Step 5 — Command registry migration dry run
If the source repo is cloned (should be at `../book-source/`), check for:
```
../book-source/Final edits/Claude-code/chatgpt_command_registry_v1.json
```
If present, read it. Produce a one-page `docs/command-migration.md` classifying each entry into the three buckets per PRD §8.5 Rule 2 (A: install as skill, B: codify in CLAUDE.md router, C: document in catalog). Do NOT install any of them yet — that happens during Phase 4 of the build. This is just the migration plan.

If the file is missing from the source repo, flag it and continue.
...
### Command Registry Migration
- Plan written to docs/command-migration.md: [yes / no / source missing]
...
<start>
Begin with Step 1 (read the PRD setup-critical sections). Produce a one-paragraph comprehension check before running setup.sh, confirming the two-repo model, the Motion policy, and the community skill review gates in your own words. Then wait for "go" before running setup.sh.
</start>
````
...
5. **Command migration plan produced** (if the source repo is cloned) — takes a minute.
...
- **Plugin install fails:** Claude Code will flag it. Try manually: `git clone <plugin-repo> ~/.claude/plugins/<name>` then `/plugin reload` inside Claude Code.

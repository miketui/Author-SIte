...
**Supersedes:** PRD v2.0 (2026-04-16), v1.0 (2026-02-16). v2.1 changelog below.

**v2.1 changelog:**
- Plugin / MCP / skill / command layers explicitly separated (new §8.5 policy).
- Tool installs are **direct and individual**. The `website-builder-setup` skill is not used (it installs legacy `framer-motion`, which conflicts with this project's Motion policy).
- UI/UX Pro Max install path clarified: native skill install first, CLI install (`uipro-cli`) as fallback.
- 21st.dev Magic: explicit MCP registration via `claude mcp add` or `.mcp.json` — not a vague skill alias.
...
- Plugin name standardized: Anthropic's security plugin is `anthropic/security-guidance` (not "Security Review" as an install alias).
- Repo audit input added: downstream session must read `Final edits/Claude-code/chatgpt_command_registry_v1.json` from the book repo as a source-of-truth input during content extraction.
- Optional community skills introduced: `pbakaus/impeccable`, `Leonxlnx/taste-skill` (behind source-verification gate), `emilkowalski/skill` (local source only).
- Router doctrine (`AUTO::DETECT+ROUTE`, `AGM`, `SALES13`, `PROMPTLIB`) codified in project-scoped `CLAUDE.md`, not installed as a plugin.
- Legacy slash commands (`/ghost`, `/godmode`, `/layered`, `/unpack`, `/livecode`, `/investigate`) migrate to project skills under `.claude/skills/`.
- **Output repo locked:** https://github.com/miketui/Author-SIte.git. All scaffolded code, `.claude/` skills, `.mcp.json`, and Vercel deploy origin point to this repo. **Source repo** https://github.com/miketui/Last.git (folder `Final edits/`) is read-only — extract content, do not push.
...
| Role | Repo | What lives there | Access mode |
|------|------|------------------|-------------|
| **Source** | `https://github.com/miketui/Last.git` | Book production files (`Final edits/final/` → EPUB, PDF, cover art). Also `Final edits/Claude-code/chatgpt_command_registry_v1.json` — Michael's slash-command registry, audit input for Phase 4. | **Read-only.** Clone locally, extract, never push back. |
| **Destination** | `https://github.com/miketui/Author-SIte.git` | All final website files — `app/`, `components/`, `lib/`, `public/`, `.claude/`, `.mcp.json`, `PRD.md`, `setup.sh`, CI config. | **Read-write.** This is where the downstream Claude Code session commits and where Vercel deploys from. |

**Clone pattern at the start of the build:**

```bash
# Destination (the project root)
git clone https://github.com/miketui/Author-SIte.git
cd Author-SIte

# Source (into a gitignored sibling directory — NEVER inside Author-SIte)
cd ..
git clone https://github.com/miketui/Last.git book-source
cd Author-SIte

# Ensure book-source is never accidentally committed
...
The `Final edits/final` folder contains the production EPUB and PDF. If the path has changed (spaces, capitalization), run `find . -iname "*.epub"` and `find . -iname "*.pdf"` to locate them.
...
Repo likely contains the 2D cover PNG/JPG. The 3D hero needs a GLB.
...
During Phase 4 (Content Extraction), the downstream session must also read — not as book content, but as project command configuration:

```
../book-source/Final edits/Claude-code/chatgpt_command_registry_v1.json
```

This is Michael's personal slash-command registry (`/ghost`, `/godmode`, `/layered`, `/unpack`, `/livecode`, `/investigate`, plus reasoning/output tokens like `BLUF`, `WHY5`, `SWOT`, `AUDIT`, `SEC`). It does not go into `lib/book-data.ts`. It drives §8.5 (Skills, Commands & MCP Policy). See that section for how each entry is categorized (install as skill, implement in `CLAUDE.md` router, or document-only).

**Rule:** Do not install any entry from this registry as an MCP server or a plugin. Entries are either (a) Claude Code skills, (b) legacy commands under `.claude/commands/*.md`, or (c) documented tokens in the router. See §8.5.
...
**Motion for React:** Section reveals at viewport threshold 0.2, stagger 60ms, ease `[0.22, 1, 0.36, 1]`. Use `/motion` skill (Motion AI Kit) to generate reveal variants.
...
- Motion Studio MCP registered with Claude Code → latest Motion docs + 330+ official examples directly accessible from the editor
- `/motion` skill (Motion AI Kit) — dedicated animation expert command
- `motion-specialist` project subagent — repeat-use animation worker
- `motion-workflow` project skill — reusable playbook invoked as slash command
- `/motion-audit` — performance scan for animations in the codebase
...
cd ..
git clone https://github.com/miketui/Last.git
cd Author-SIte
...
- `../Last/Final edits/` exists and contains the EPUB, PDF, and `Claude-code/chatgpt_command_registry_v1.json`.
...
Two inputs feed this phase: (a) the book EPUB/PDF for all chapter content, and (b) the command registry that catalogs Michael's slash-command doctrine.

**4.1 Book content extraction (primary):**

```
/superpowers research "parse ../Last/Final edits/final/ EPUB, extract TOC + pull-quotes + chapter openings per schema in PRD §2.4, write lib/book-data.ts"
```

**4.2 Command registry audit (doctrine migration input):**

```
/superpowers research "read ../Last/Final edits/Claude-code/chatgpt_command_registry_v1.json as source-of-truth for Michael's slash-command system. Cross-reference against the PRD §8.5 policy. Produce a migration report at docs/command-migration.md listing: (A) commands to install as project skills under .claude/skills/ (ghost, godmode, layered, unpack, livecode, investigate), (B) router doctrine to codify in CLAUDE.md (AUTO::DETECT+ROUTE, AGM, SALES13, PROMPTLIB), and (C) reasoning/output tokens to document in docs/command-catalog.md (BLUF, WHY5, SWOT, AUDIT, SEC, and the rest)."
```
...
- `docs/command-catalog.md` — token reference with behaviors documented.
...
Motion+ subscribers (adds token-gated features):
...
Use `components/motion/Reveal.tsx` as the canonical reveal wrapper for section reveals. Hand-author the big set pieces (hero reveal, chapter ring rotation, pricing card spring). Use `/motion` (if Motion+ installed) or `motion-specialist` subagent otherwise.

Example invocations (after setup):

```
Use /motion to build a refined card hover interaction for the pricing tile. Subtle, premium, mobile-safe.
Use motion-specialist to convert /about scroll reveals into a reusable Reveal wrapper.
Use /motion to add staggered section reveals on the home page. Keep it accessible.
```

**8.8 Optional: install Motion audit for performance triage.**

If available via the AI Kit:

```
/motion-audit src/components
```

Scans Motion + CSS animations for performance issues (layout thrash, repaints, oversized transforms). Run before launch and after any major motion work.
...
```
/claude-mem save --project michaeldavidjr-site --topics "ACISS palette, 3D book hero, charge-now pre-order, MailerLite group 182303148544623709, RELEASE_DATE cron"
```
...
The file `../book-source/Final edits/Claude-code/chatgpt_command_registry_v1.json` is Michael's personal slash-command system. During Phase 4, the Superpowers plugin reads it and produces `docs/command-migration.md` classifying each entry into one of three buckets:
...
- `docs/command-migration.md` exists and accounts for every entry in the source command registry.
...
3. **Verification performed** — Lighthouse scores, Stripe test results, email deliverability, 3D performance, WCAG pass.
4. **API keys still pending** — any in §9 not yet supplied by Michael.
5. **Launch-date go/no-go checklist** — from §12 T-7 and T-1.
6. **claude-mem persistence** — confirm key decisions saved.
...
- Front cover PNG (high-res, from book production files in the GitHub repo)
- Back cover PNG (same)
...
The handoff prompt the downstream session should be invoked with:

```
Read PRD.md in full. Build michaeldavidjr.beauty per the spec. Work the 12 phases in §8 in order. Report progress in the format specified in §16. Use all six plugins and the full design stack. When you hit any [CONFIRM WITH MICHAEL] item, list it at the bottom of your response and keep moving with a documented assumption. Do not add features not in the PRD. Do not swap libraries without flagging. Do not skip the security and code review phases. Do not deploy to production until T-7 QA is green.
```
